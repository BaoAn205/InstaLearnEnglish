...
public class ST4_DragAndDropLuggageFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.st4_fragment_drag_and_drop_luggage, container, false);
    }
    ...
    private void updateGameProgress() {
        ...
        userDocRef.update("station4_completed_games", FieldValue.arrayUnion("DRAG_AND_DROP_LUGGAGE"))
                    ...
                    if (documentSnapshot.exists()) {
                        ...
                        if (completedGames != null && completedGames.size() >= 7) {
                            if (documentSnapshot.getLong("currentLevel") == 4L) {
                                userDocRef.update("currentLevel", 5L).addOnSuccessListener(aVoid1 -> showWinDialog(true));
                            } else {
                                showWinDialog(false);
                            }
                        } else {
